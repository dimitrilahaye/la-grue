...
// Crane SVG body (no background) for use on coloured backgrounds
const CRANE_BODY = `
...
async function run() {
  ...
  await sharp(svgBuffer).resize(32, 32).png().toFile(fav32);
  console.log('✓ favicon-32x32.png');

  // Apple touch icon (180×180)
  const touch = path.join(assetsDir, 'apple-touch-icon.png');
  await sharp(svgBuffer).resize(180, 180).png().toFile(touch);
  console.log('✓ apple-touch-icon.png');

  // Splashscreens (portrait, @2x device pixels)
  const splashes: Array<{ w: number; h: number; label: string }> = [
    { w: 750,  h: 1334, label: 'iPhone SE / 8' },
    { w: 1170, h: 2532, label: 'iPhone 12/13/14' },
    { w: 1290, h: 2796, label: 'iPhone 14 Pro Max / 15 Plus' },
  ];
